...
from store.serializers.product import ToolAdminSerializer
...
class ToolAdminTest(TestCase):
    ...
    def setUp(self):
        ...
        def test_get_all_tools(self):
            ...
            serializer = ToolAdminSerializer(tools, many=True)
            self.assertEqual(response.data, serializer.data)
            self.assertEqual(response.status_code, status.HTTP_200_OK)

        def test_get_valid_single_tool(self):
            response = client.get(
                reverse('admin_tools_detail', kwargs={'pk': Tool.objects.all().first()}))
            tool = Tool.objects.all().first()
            serializer = ToolAdminSerializer(tool)
            self.assertEqual(response.data, serializer.data)
            self.assertEqual(response.status_code, status.HTTP_200_OK)

        def test_get_invalid_single_tool(self):
            response = client.get(
                reverse('admin_tools_detail', kwargs={'pk': 1}))
