...
class GetCountCart(APIView):
 def get(self, request, format='json'):
  count = 0
  userToGetCart = request.user
  count = count + PlantCartModel.objects.filter(user=userToGetCart, is_approved=False).count()
  count = count + ToolCartModel.objects.filter(user=userToGetCart, is_approved=False).count()
 ...
 def delete(self, request, format='json'):
  ...
  if serializer.is_valid():
   ...
   try:
    ...
    return response.Response("Can NOT Remove This Plant", status=status.HTTP_400_BAD_REQUEST)
   return response.Response("Plant Removed",  status=status.HTTP_200_OK)
...
class RemoveToolFromCart(APIView):
 def post(self, request, format='json'):
  ...
  if serializer.is_valid():
   ...
   return response.Response("Tool Removed",  status=status.HTTP_200_OK)
